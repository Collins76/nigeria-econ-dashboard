import { Moon, Sun, Contrast, Bell, Download, Menu, X } from 'lucide-react'
...
export default function Header({ darkMode, toggleDark, highContrast, setHighContrast, activeAlerts }: Props) {
    ...
    >
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-3">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm font-medium" aria-label="Main navigation">
          {['Overview', 'Indicators', 'Comparison', 'Reports'].map(label => (
            <a
              key={label}
              href="#"
              className="px-3 py-1.5 rounded-md text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              {label}
            </a>
          ))}
        ...
        {/* Mobile menu */}
        <button
          className="md:hidden p-2 rounded-md text-slate-500 dark:text-slate-400"
      ...
      {/* Mobile nav */}
      {menuOpen && (
        <nav className="md:hidden border-t border-slate-200 dark:border-slate-700 px-4 py-2">
          {['Overview', 'Indicators', 'Comparison', 'Reports'].map(label => (
            <a
              key={label}
              href="#"
